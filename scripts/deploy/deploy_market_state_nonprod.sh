...
SERVICE_NAME="${SERVICE_NAME:-poly-orchestrator-nonprod-market-state}"
...
IMAGE_REPO="${ECR_REGISTRY}/poly-orchestrator/nonprod/market-state"
...
docker buildx build \
  ...
  -f services/market-state/Dockerfile \
  -t "$IMAGE_URI" \
  --push \
  .
